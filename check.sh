...
# FailWarden Orchestrator quality gate.
#
# This gate is intended to prove more than formatting hygiene. It checks staged
# patch quality, secrets exposure, dependency risk, config syntax, docs sanity,
# and the Python test suite used by this repository.
...
echo "[setup] Upgrading pip and installing project tooling"
if ! "$VENV_PYTHON" -m pip install -U pip >/dev/null 2>&1; then
  echo "[setup] WARN: Could not upgrade pip; continuing with existing pip."
fi
if ! "$VENV_PYTHON" -m pip install -e '.[dev]' >/dev/null 2>&1; then
...
patterns = [
    ...
    (
        "debug print",
        re.compile(
            r"^\+\s*.*\b(console\.log|print\(|dbg!|debugger;|fmt\.Println|"
            r"System\.out\.println)\b"
        ),
    ),
    ...
    (
        "private key marker",
        re.compile(
            r"^\+\s*.*(BEGIN [A-Z0-9 ]*PRIVATE KEY|AKIA[0-9A-Z]{16}|"
            r"ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,})"
        ),
    ),
    (
        "confidential placeholder drift",
        re.compile(
            r"^\+\s*.*\b(password|passwd|token|secret|api[_-]?key)\b\s*[:=]\s*"
            r"['\"]?[A-Za-z0-9/_+=.-]{8,}"
        ),
    ),
]
...
dangerous_re = re.compile(
    ...
    r".*:Zone\.Identifier$|.*\.local\..*|id_rsa(|\.pub)$|id_ed25519(|\.pub)$)",
    re.IGNORECASE,
)
artifact_re = re.compile(
    r"\.(zip|tar|tgz|gz|bz2|xz|7z|rar|sqlite|sqlite3|db|dump|bak|csv|tsv|"
    r"parquet|png|jpg|jpeg|gif|bmp|pdf|doc|docx|xls|xlsx|ppt|pptx|mp4|mov|"
    r"avi|mkv|wav|mp3)$",
    re.IGNORECASE,
)
for raw in proc.stdout.splitlines():
...
run_git_checks() {
  ...
  if [[ "$FAIL_ON_BINARY_STAGED" == "1" ]]; then
    local binary_staged
    binary_staged="$(
      git diff --cached --numstat --diff-filter=AM | awk '$1 == "-" || $2 == "-" {print $3}'
    )"
...
if have_tool "bandit"; then
  ...
  "$VENV_PYTHON" -m bandit -r . \
    -x "./.venv,./venv,./.git,./__pycache__,./.pytest_cache,./.ruff_cache,./build,./dist" \
...
for result in data.get("results", []):
    ...
    conf = (result.get("issue_confidence") or "").lower()
    if order.get(sev, 0) >= min_sev_n and order.get(conf, 0) >= min_conf_n:
        bad.append(result)

if bad:
    print(
        f"FAIL: Bandit found {len(bad)} issue(s) at severity>={min_sev} "
        f"and confidence>={min_conf}."
    )
    for result in bad[:25]:
        print(
            f"- {result.get('filename')}:{result.get('line_number')} "
            f"{result.get('test_id')} {result.get('issue_text')} "
            f"(sev={result.get('issue_severity')}, "
            f"conf={result.get('issue_confidence')})"
        )
    if len(bad) > 25:
...
if command -v gitleaks >/dev/null 2>&1; then
  ...
  if [[ "$GITLEAKS_RC" != "0" ]]; then
    ...
    if [[ "$GITLEAKS_FAIL_ON_FINDINGS" == "1" ]]; then
      echo "FAIL: gitleaks found potential secrets or could not complete."
      exit 13
    fi
...
if have_tool "pip_audit"; then
  run_pip_audit() {
    ...
    if [[ "$rc" != "0" ]] && grep -qiE \
      ...
      echo "[deps] WARN: pip-audit could not reach vulnerability service; skipping."
      if [[ -n "$reason" ]]; then
  ...
  if [[ "$PIP_AUDIT_FAIL_ON_VULNS" == "1" && "$AUDIT_RC" != "0" ]]; then
    echo "FAIL: pip-audit reported vulnerabilities."
    exit 11
  fi
...
if [[ "$ENABLE_SHELLCHECK_IF_AVAILABLE" == "1" ]]; then
  ...
  elif [[ "$FAIL_ON_MISSING_OPTIONAL_TOOLS" == "1" ]] && \
    find . -path './.git' -prune -o -type f -name '*.sh' -print | grep -q .; then
...
if [[ "$ENABLE_MARKDOWNLINT_IF_AVAILABLE" == "1" ]]; then
  if have_command markdownlint; then
    ...
    find . -path './.git' -prune -o -type f -name '*.md' -print | grep -q .; then
